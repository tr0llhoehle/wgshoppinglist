		...
		connection.query('SELECT * FROM shopping_lists WHERE shopping_list_id = '+connection.escape(req.params.id)+' LIMIT 1', function(err, rows, fields) {
			if(rows[0] != null) {
				if(rows[0].wg_id == req.user.wg) {
					connection.query('SELECT item_id, description, insert_date, checked, user_id FROM items WHERE shopping_list_id = '+connection.escape(req.params.id)+' AND removed = 0 AND purchase_id IS NULL ORDER BY checked,description', function(err, itemrows, fields) {
